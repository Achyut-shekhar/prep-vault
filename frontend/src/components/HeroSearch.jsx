...
const HeroSearch = ({ onSearch }) => {
    ...
    return (<section id="search" className="relative overflow-hidden py-20 md:py-32">
       ...
       <div className="container mx-auto px-4">
         <div className="mx-auto max-w-3xl text-center">
           ...
           <form onSubmit={handleSearch} className="mb-8">
             <div className="relative mx-auto max-w-2xl">
               <div className="relative flex items-center overflow-hidden rounded-2xl border border-border bg-card shadow-lg shadow-glow transition-shadow focus-within:shadow-xl">
                 ...
                 <Input type="text" placeholder="Search companies, exams, or topics..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-14 border-0 bg-transparent pl-12 pr-32 text-base focus-visible:ring-0 md:text-lg"/>
                 <Button type="submit" size="lg" className="absolute right-2 bg-primary hover:bg-primary/90">
                   Search
                 </Button>
               </div>
             </div>
           ...
           <div className="flex flex-wrap items-center justify-center gap-2">
             ...
             {trendingSearches.map((item) => (<button key={item.name} onClick={() => handleTrendingClick(item.name)} className="group relative inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-sm font-medium transition-all hover:border-primary/50 hover:bg-primary/5">
                 {item.name}
                 {item.hot && (<span className="flex h-2 w-2">
