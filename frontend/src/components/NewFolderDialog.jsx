         ...
         <form onSubmit={handleSubmit} className="space-y-6 pt-4">
           ...
           <div className="space-y-2">
             ...
             <div className="flex flex-wrap gap-2">
               {presetNames.map((name) => (<button key={name} type="button" onClick={() => setFolderName(name)} className={`rounded-full border px-3 py-1 text-sm transition-colors ${folderName === name
           ...
           <div className="rounded-lg border border-border bg-secondary/30 p-4">
            ...
            : "Only you can see this folder"}
               ...
               <Switch checked={isPublic} onCheckedChange={setIsPublic} className="data-[state=checked]:bg-accent"/>
             </div>
 
             {isPublic && (<div className="mt-3 rounded-md bg-accent/10 px-3 py-2 text-sm text-accent">
           ...
           <div className="flex gap-3 pt-2">
             ...
             <Button type="submit" className="flex-1 bg-primary hover:bg-primary/90" disabled={!folderName.trim()}>
               Create Folder
             </Button>
           </div>
         </form>
